...
def show_welcome(parent: Gtk.Widget, on_continue):
    """Route a: no previous detection — explain the app before scanning."""
    dialog = Adw.AlertDialog.new(
        "Welcome to Ultimate Color Switcher!",
        "This is the first time you're running the app in this project.\n\n"
        "We're going to scan the files configured in config.json for "
        "colors (hex and rgb), and then you'll be able to build a new palette and "
        "map each detected color to a color from that palette, to apply it "
        "to your dotfiles.",
    )
...
def show_configure_files_onboarding(parent: Gtk.Widget, config, on_done):
    ...
    dialog = Adw.AlertDialog.new(
        "Which files do you want to recolor?",
        "Ultimate Color Switcher looks for and replaces colors in the files on a list. "
        "It's still empty.\n\n"
        "I can automatically search ~/.config for files that have colors and build you "
        "a starting point.\n\n"
        "⚠ It's only a starting point: it may include hex-looking values that aren't colors "
        "(like 0xADDR addresses) or files you don't want to modify. You'll be able to review "
        "and remove them before confirming.",
    )
    dialog.add_response("external", "Edit config.json")
    dialog.add_response("manual", "Add by hand")
    dialog.add_response("scan", "Search ~/.config")
...
def run_config_scan_flow(parent: Gtk.Widget, config, on_done):
    ...
    box.append(spinner)
    box.append(Gtk.Label(label="Searching for colors in ~/.config…"))
    ...
    def on_scan_done(found):
        ...
        if not found:
            info = Adw.AlertDialog.new(
                "No results",
                "No files with colors were found in ~/.config. You can add them by hand any time.",
            )
            info.add_response("ok", "Got it")
...
def show_config_scan_results(parent: Gtk.Widget, config, found: list, on_done):
    ...
    folder), each holding a checkbox row per file. "Add selected"
    ...
    pref_group = Adw.PreferencesGroup(
        title="Files with colors found",
        description="Uncheck what you don't want. You can turn off a whole folder or "
                    "individual files. May include hex-looking values that aren't colors, "
                    "or files you don't want touched.",
    )
    ...
    for folder, files in groups:
        ...
        if folder == config_root:
            title = f"{cfg.to_home_relative(folder)}  ·  loose files"
        else:
    ...
    dialog = _build_dialog_shell(
        "Review found files", page, [add_button],
...
def show_stale_detect(parent: Gtk.Widget, diff: dict, on_choice):
    """Route c: a fresh detect differs from the saved one. on_choice receives
    "rescan" or "keep"."""
    lines = []
    if diff["added"]:
        lines.append(f"New: {len(diff['added'])} color(s)")
    if diff["removed"]:
        lines.append(f"No longer present: {len(diff['removed'])} color(s)")
    body = (
        "The detected colors changed since the last time you opened the app.\n\n"
        + "\n".join(lines)
        + "\n\nA rescan and a new mapping are recommended. If you keep the "
        "previous detection, the mapping might not match your current files."
    )
    dialog = Adw.AlertDialog.new("The colors changed", body)
    dialog.add_response("keep", "Keep the previous one")
    dialog.add_response("rescan", "Rescan (recommended)")
...
def show_restart_actions_onboarding(parent: Gtk.Widget, config, on_done):
    ...
    but with an explicit "Continue" action so it can't go unnoticed before
    ...
    intro = Gtk.Label(
        label="These are the ones you'll probably need to restart to pick up the new colors. "
              "You can adjust them now or later from the menu (⋮).",
